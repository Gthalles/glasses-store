...
function Item(item: IItem) {
	return (
		<Container flex direction="column" margin="16px auto" width="calc(100% - 32px)" height="100%">
			<S.Image src={item.image} alt="Produto popular" />

			<Button text="Ver mais" margin="8px auto" width="95px" height="30px" borderRadius="5px" backgroundColor={colors.secondary} />
		</Container>
	);
}
...
function BestSellers() {
	...
	return (
		<Carousel>
			{
				items.map((item: IItem) => <Item key={item.name} image={item.image} name={item.name} description={item.description} />)
			}
		</Carousel>
	);
}
